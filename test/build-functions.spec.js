...
describe('buildFunctions', () => {
	...
	it('can use callback based methods', () => {
		...
		const functions = buildFunctions({ doAction });
	...
	it('can call sync methods', () => {
		...
		const functions = buildFunctions({ getSimple });
		return functions
			.getSimple(false)
			.then(out => {
	...
	it('call read context', () => {
		...
		const functions = buildFunctions({ usesContext }, { context: { value: 1, unchanged: 10 } });
		return Promise.all([
